...
#define COMM_END_EXT_TIMED_PROG	0x0A
#define COMM_BULK_ERASE		0x09
#define COMM_ROW_ERASE		0x11

#define ENTER_PROGRAM_KEY	0x4D434850

void pic16f150x::enter_program_mode(void)
{
	int i;
...
void pic16f150x::exit_program_mode(void)
{
...
void pic16f150x::send_cmd(uint8_t cmd, unsigned int delay)
{
	int i;
...
uint16_t pic16f150x::read_data(void)
{
	uint8_t i;
...
void pic16f150x::write_data(uint16_t data)
{
	int i;
	data <<= 1;
...
/* set Table Pointer */
void pic16f150x::reset_mem_location(void)
{
	send_cmd(COMM_RESET_ADDR, DELAY_TDLY);
}

/* Read PIC device id word */
bool pic16f150x::read_device_id(void)
{
	uint16_t id;
...
/* Blank Check */
uint8_t pic16f150x::blank_check(void)
{
...
/* Bulk erase the chip */
void pic16f150x::bulk_erase(void)
{
	send_cmd(COMM_RESET_ADDR, DELAY_TDLY);
...
void pic16f150x::read(char *outfile, uint32_t start, uint32_t count)
{
	...
	/* Config Word 2 */
	uint16_t mask = 0x3E03;

	addr++;
	send_cmd(COMM_INC_ADDR, DELAY_TDLY);
	send_cmd(COMM_READ_FROM_PROG, DELAY_TDLY);

	if(detailed_subfamily == SF_PIC16F1501_7)
		mask = 0x2E03;
...
void pic16f150x::write(char *infile)
{
	int i;
	...
	if(!flags.noverify){
		 ...
		 * We will ignore LVP bit in Configuration Fuse by using 0x3E03 mask.
		 */
...
void pic16f150x::dump_configuration_registers(void)
{
	send_cmd(COMM_LOAD_CONFIG, DELAY_TDLY);
	write_data(0x00);
